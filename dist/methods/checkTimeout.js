// Import ms
import ms from 'ms';
export const checkTimeout = (interaction, db, cmdName, timeoutMs, userData) => {
    // Get index
    ...
    if (index !== -1) {
        ...
        if (Number(timeout.time + timeoutMs) - Date.now() <= 0) {
            ...
            // Send error message
            interaction.reply(`<@${interaction.user.id}> You can use this command again in ${ms(Number(timeout.time) + timeoutMs - Date.now())}`);
            return true;
        }
    }
    // Check if index is -1
