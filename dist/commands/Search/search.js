    ...
    async execute(client, interaction) {
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case "wiki":
                // Search in wiki
                axios
                    .get(`https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&exintro=&explaintext=&titles=${query}`)
                    .then((response) => {
                    // Get data
                        ...
                        interaction.reply({
                            embeds: [
                                new MessageEmbed()
                                    ...
                                    .setDescription(`${result.extract.substr(0, result.extract.length >= 500 ? 497 : result.extract.length)}...`)
                                    .setColor(0x00ae86)
            ...
            case "wikidata":
                // Search in wikidata
                axios
                    .get(`https://www.wikidata.org/w/api.php?action=wbsearchentities&search=${query}&language=en&format=json&type=item`)
                    .then((response) => {
                    // Get data
                    const { data } = response;
                    // If no results
