    ...
    async execute(client, interaction) {
        ...
        if (!job) {
            ...
            const embed = new MessageEmbed()
                ...
                // Set description
                .setDescription(jobs
                .map((j) => `${Number(user.level) < Number(j.minimumLevel) ? "🔒" : "✅"} **${j.name}** - *${j.description}*\nIncome: $${j.income}`)
                .join("\n\n"))
                // Set color
                .setColor("RED");
            // Send embed
            ...
            if (!jobs.find((j) => j.name.toLowerCase() === job.toLowerCase())) {
                ...
                if (Number(user.level) < Number(jobs.find((j) => j.name.toLowerCase() === job.toLowerCase()).minimumLevel)) {
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setTitle("Jobs")
                        // Set description
                        .setDescription(`You need to be at least level ${jobs.find((j) => j.name.toLowerCase() === job.toLowerCase()).minimumLevel} to choose this job!`)
                        // Set color
                        .setColor("RED");
                    // Send embed
                    interaction.reply({ embeds: [embed] });
