...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        const embed = new MessageEmbed()
            ...
            .setDescription(`Welcome to Tic-Tac-Toe!\n${user.username} is playing with you.\n\n${game.board
        ...
        collector.on("collect", async (/** @type {import('discord.js').MessageComponentInteraction}*/ i) => {
            ...
            // Set message embed
            embed
                .setDescription(`${user.username} is playing with you.\n\n${game.board
                .map((row) => row.map((cell) => (cell === 0 ? "❓" : cell === 1 ? "❌" : "⭕")).join(" | "))
                .join("\n")}
            `)
                .setFooter({
            ...
            if (game.checkIfWin() > 0) {
                ...
                return interaction.editReply({
                    content: `🎉 Congratulations ${game.checkIfWin() === 1 ? interaction.user.username : user.username}! You won!`,
                    embeds: [embed],
                    components: [],
                });
            }
