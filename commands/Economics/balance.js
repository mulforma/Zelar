...
module.exports = {
  ...
  async execute (client, interaction) {
    ...
    // Get user balance
    client.db.select("coin").from("user").where("userId", user.id).andWhere("serverId", interaction.guild.id).then(async (rows) => {
      // Check if user has balance
      console.log(rows)
      if (rows.length === 0) {
        // Send error message
        interaction.reply(`Oops! \`${user.username}\`'s profile is not set up yet.\nPlease use \`/profile\` to set up profile.`);
        // Return
        return;
      }
